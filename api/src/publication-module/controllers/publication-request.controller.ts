...
@Controller('publications/request')
...
export class PublicationRequestController {
	...
	async getPublicationDetail(@Param('id') id: number) {
		return this.publicationRequestService.getPublicationDetail(id);
	}
	...
	async listPublicationRequests(
		...
		const [publications, count] = await this.publicationRequestService.getPublicationRequests(
			pagination,
			sorting,
			status,
			search
	...
	async approve(@Param('id') id: number, @RequestUser() user: UserDto, @Body() body: ApprovePublicationDto) {
		return this.publicationRequestService.approvePublication(id, user.id, body);
	}
	...
	async reject(@Param('id') id: number, @RequestUser() user: UserDto, @Body() body: ApprovePublicationDto) {
		return this.publicationRequestService.rejectPublication(id, user.id, body);
	}
	...
	async exportPublications(
		...
		const { headers, rows } = await this.publicationRequestService.exportPublications(
			status,
			search,
			startDate,
			endDate,
